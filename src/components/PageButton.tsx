  ...
  return (
    <div className="fixed  top-60 right-2 z-20 flex flex-col gap-2 ">
      <button
        ...
        className="bg-white overflow-hidden w-16 h-[70px] rounded-md flex flex-col gap-1 text-[#33475b] justify-center items-center shadow-testShadow overflow-x-hidden group cursor-pointer "
      >
        <div className="flex justify-center items-center">
          {userInfo && userInfo?.picture ? (
            ...
            <Link href={"/auth/login"}><FaUser className="text-2xl -translate-x-12 group-hover:translate-x-3 transition-transform duration-200 " /></Link>
          )}
          {userInfo && userInfo?.picture ? (
            <Image
              src={userInfo?.picture}
            ...
            <FaUser className="text-xl -translate-x-3 group-hover:translate-x-12 transition-transform duration-200" />
          )}
        </div>
        {
          userInfo?.name ?  <p className="text-xs font-semibold">{userInfo?.name?.slice(0,6)}</p> : <Link href={"/auth/login"} className="text-xs font-semibold">Login</Link>
        }
